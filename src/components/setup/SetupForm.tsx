...
export function SetupForm({
  ...
  const [connected, setConnected] = useState(
    youtubeConnected || Boolean(initialChannelId ?? profile?.youtubeChannelId),
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-12">
      <section className="space-y-4">
        ...
        <button
          ...
          disabled={syncing}
          className="text-sm text-[#2a6f6a] underline-offset-2 hover:underline disabled:opacity-50"
        >
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[#1a1612]/60">複数選択できます</p>
        </div>
      ...
      <button
        type="submit"
        disabled={saving}
        className="w-full rounded-full bg-[#1a1612] px-6 py-4 text-sm font-semibold text-[#f4f0e8] transition hover:opacity-90 disabled:opacity-60 sm:w-auto sm:min-w-[14rem]"
      >
